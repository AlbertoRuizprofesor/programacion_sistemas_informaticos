"""
En el bloque principal del programa definir un diccionario que 
almacene los nombres de paises como clave y como valor la cantidad de habitantes. 
Implementar una función para mostrar cada clave y valor.
"""

def mostrar_poblaciones(diccionario_paises):
    """
    Recorre el diccionario recibido y muestra
    cada país junto con su población.
    """
    for pais in diccionario_paises:
        print(pais, diccionario_paises[pais])


# Bloque principal

poblacion_paises = {
    "argentina": 40000000,
    "españa": 46000000,
    "brasil": 190000000,
    "uruguay": 3400000
}

mostrar_poblaciones(poblacion_paises)
